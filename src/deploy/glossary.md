# R&D Acronym & Term Glossary

Neutral reference definitions for acronyms and product/component names that recur
across the R&D ticket corpus. Entries are short, factual, and vendor/component
oriented. They are **not** phrased against any specific ticket or expected answer —
a Knowledge Assistant should treat these as one grounding source among many (corpus
co-occurrence is the other), hedge when a term is ambiguous in context, and always
cite.
...
- **LoopSense.** Inductive-loop vehicle-detection subsystem used for presence and
...
- **Veridyne.** Sensor vendor whose quartz/piezo strip sensors are used in WIM
  installations. "Veridyne sensor" typically refers to a WIM load sensor.
...
- **OVC — overview camera.** Overview camera unit (OVC designation) used in roadside
  imaging.
...
- **Aptix / Lumex.** Vendors associated with ALPR cameras and readers — Aptix is the
  legacy reader, Lumex the newer replacement camera.
- **CamView.** Camera viewer / SDK tooling used to view or configure machine-vision
  camera streams (GigE Vision cameras from vendor Optivue).
